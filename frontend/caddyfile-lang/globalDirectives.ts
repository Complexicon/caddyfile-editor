...
export const descriptions: Descriptions = {
    ...
    auto_https:
        '**auto_https** configure automatic HTTPS. It can either disable it entirely (`off`) or disable only HTTP-to-HTTPS redirects (`disable_redirects`). See the [Automatic HTTPS](https://caddyserver.com/docs/automatic-https) page for more details.',
    cert_issuer: '**cert_issuer** defines the issuer (or source) of TLS certificates.',
    servers:
        "**servers** Customizes [HTTP servers](https://caddyserver.com/docs/json/apps/http/servers) with settings that potentially span multiple sites and thus can't be rightly configured in site blocks. These options affect the listener/socket, or other behavior beneath the HTTP layer.",
};